                            ...
                            size_t output_max_len )
{
    ...
    MBEDTLS_MPI_CHK(kapi_hash_start( &hashid, hisi_type, NULL, 0 ));
    MBEDTLS_MPI_CHK(kapi_hash_update( hashid, (hi_u8*)label, label_len, HASH_CHUNCK_SRC_LOCAL ));
    MBEDTLS_MPI_CHK(kapi_hash_finish( hashid, lhash, sizeof(lhash), &hisi_hlen ));
                              ...
                              size_t offset )
{
    ...
    for( i = 0; i < total; i++ )
    {
        ...
        for( n = 0; n < total - 1; n++ )
        {
            unsigned char cur = buf[n];
            unsigned char next = buf[n+1];
            buf[n] = if_int( no_op, cur, next );
        }
                               ...
                               const unsigned char *sig )
{
    ...
    if ( ret != 0 )
        goto exit;
    ret = kapi_hash_finish( hashid, result, sizeof(result), &hisi_hlen );
    if ( ret != 0 )
        goto exit;
