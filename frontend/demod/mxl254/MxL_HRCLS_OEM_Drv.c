		...
		sout[0] = (UINT8)((regData >> 8) & 0x00ff);
        	sout[1] = (UINT8)(regData & 0x00ff);
              ...
              if(HI_STD_I2C_NUM >i2cIndex)
         	{
                ...
                //s32Ret = s_tuner_pI2cFunc->pfn_i2c_write(i2cIndex, i2cAddress, regAddr,2, sout, 2);
         	...
         	{
         	    s32Ret = hi_drv_gpio_i2c_write_ext(i2cIndex,i2cAddress, regAddr, 2, sout, 2);
		        //s32Ret = s_tuner_pGpioI2cFunc->pfn_gpio_i2c_write_ext(i2cIndex,i2cAddress, regAddr, 2, sout, 2);
         	    if(HI_SUCCESS != s32Ret)
         	    {
         	        return MXL_FAILURE;
         	    }
	...
	if (user_data)
	{
		...
		readAddr = (0xff << 24) | (0xfb << 16) | regAddr;
                if(HI_STD_I2C_NUM > i2cIndex)
 		   {
			    ...
			    //s32Ret = s_tuner_pI2cFunc->pfn_i2c_read(i2cIndex, i2cAddress, readAddr, 4, sout, 2);
 		        if(HI_SUCCESS != s32Ret)
 		        {
 		            *dataPtr = 0;
 		            return MXL_FAILURE;
 		    ...
 		    {
 		        s32Ret = hi_drv_gpio_i2c_read_ext(i2cIndex, i2cAddress,readAddr, 4, sout, 2);
			    //s32Ret = s_tuner_pGpioI2cFunc->pfn_gpio_i2c_read_ext(i2cIndex, i2cAddress,readAddr, 4, sout, 2);
 		        if(HI_SUCCESS != s32Ret)
 		        {
 		            *dataPtr = 0;
 		            return MXL_FAILURE;
		...
		if(HI_STD_I2C_NUM >i2cIndex)
         	{
         	   ...
         	   //s32Ret = s_tuner_pI2cFunc->pfn_i2c_write(i2cIndex, i2cAddress, regAddr,2, bufPtr, bufSize);
         	...
         	{
         	    s32Ret = hi_drv_gpio_i2c_write_ext(i2cIndex,i2cAddress, regAddr, 2, bufPtr, bufSize);
		        //s32Ret = s_tuner_pGpioI2cFunc->pfn_gpio_i2c_write_ext(i2cIndex,i2cAddress, regAddr, 2, bufPtr, bufSize);
         	    if(HI_SUCCESS != s32Ret)
         	    {
         	        return MXL_FAILURE;
         	    }
...
MXL_STATUS_E MxLWare_HRCLS_OEM_ReadBlock(UINT8 devId, UINT16 regAddr, UINT16 readSize, UINT8 *bufPtr)
{
	...
	if (user_data)
	{
		...
		if(HI_STD_I2C_NUM > i2cIndex)
		{
		    ...
		    //s32Ret = s_tuner_pI2cFunc->pfn_i2c_read(i2cIndex, i2cAddress, readAddr, 4, bufPtr, readSize);
		    if(HI_SUCCESS != s32Ret)
		    {
		        return MXL_FAILURE;
		    }
		...
		{
		    s32Ret = hi_drv_gpio_i2c_read_ext(i2cIndex, i2cAddress,readAddr, 4, bufPtr, readSize);
		    //s32Ret = s_tuner_pGpioI2cFunc->pfn_gpio_i2c_read_ext(i2cIndex, i2cAddress,readAddr, 4, bufPtr, readSize);
		    if(HI_SUCCESS != s32Ret)
		    {
		        return MXL_FAILURE;
		    }
...
MXL_STATUS_E MxLWare_HRCLS_OEM_ReadBlockExt(UINT8 devId, UINT16 cmdId, UINT16 offset, UINT16 readSize, UINT8 *bufPtr)
{
  ...
  //MXL_STATUS_E status = MXL_SUCCESS;
	...
	if (user_data)
	{
		...
		if(HI_STD_I2C_NUM >i2cIndex)
		{
		    s32Ret = hi_drv_i2c_write(i2cIndex, i2cAddress, regAddr,4, bufPtr, 2);
		   //s32Ret = s_tuner_pI2cFunc->pfn_i2c_write(i2cIndex, i2cAddress, regAddr,4, u8data, 2);
		...
		{
			s32Ret = hi_drv_gpio_i2c_write_ext(i2cIndex,i2cAddress, regAddr, 4, bufPtr, 2);
			//s32Ret = s_tuner_pGpioI2cFunc->pfn_gpio_i2c_write_ext(i2cIndex, i2cAddress, regAddr,4, u8data, 2);
			if(HI_SUCCESS != s32Ret)
			{
				return MXL_FAILURE;
			}
		...
		//read
		  ...
		  if(HI_STD_I2C_NUM > i2cIndex)
		  {
		  	  s32Ret = hi_drv_i2c_read_directly(i2cIndex, i2cAddress, readAddr, 0, bufPtr, readSize);
			  //s32Ret = s_tuner_pI2cFunc->pfn_i2c_read_directly(i2cIndex, i2cAddress, readAddr, 0, bufPtr, readSize);
			  if(HI_SUCCESS != s32Ret)
			  {
				  return MXL_FAILURE;
			  }
		  ...
		  {
		   	  s32Ret = hi_drv_gpio_i2c_read_ext_directly(i2cIndex, i2cAddress,readAddr, 0, bufPtr, readSize);
			  //s32Ret = s_tuner_pGpioI2cFunc->pfn_gpio_i2c_read_ext_directly(i2cIndex, i2cAddress,readAddr, 0, bufPtr, readSize);
			  if(HI_SUCCESS != s32Ret)
			  {
				  return MXL_FAILURE;
			  }
...
/*
    ...
    {
        LOG_ERR_PRINT("error occured while opening file nvram50.bin, exiting...\n");
        return MXL_FAILURE ;
    }
    old_fs = get_fs();
    set_fs(KERNEL_DS);
