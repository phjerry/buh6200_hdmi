...
extern MXL_BOOL_E  g_supported;
extern hi_bool g_mxl254_inin_flag;
...
MXL_STATUS_E mxl_downloadFirmware(MXL_BOOL_E * xptSupported)
{
	...
	************************************************************************************/
	...
	LOG_INFO_PRINT("[HRCLS][%d] {%s} Firmware download OK\n", HRCLS_DEVICE_ID, __FUNCTION__);
	...
	************************************************************************************/
	...
	{
		result = MXL_SUCCESS;
		LOG_INFO_PRINT("[HRCLS][%d] {%s} Firmware ver. %d.%d.%d.%d-RC%d\n", HRCLS_DEVICE_ID, __FUNCTION__,
	...
	{
		LOG_ERR_PRINT("[HRCLS][%d] {%s} Firmware version read FAILED\n", HRCLS_DEVICE_ID, __FUNCTION__);
	}
	}
	else
	{
		LOG_ERR_PRINT("[HRCLS][%d] {%s} Firmware download FAILED (error code: %d)\n", HRCLS_DEVICE_ID, __FUNCTION__, status);
	}
...
MXL_STATUS_E mxl_checkVersion(/*@out@*/ MXL_HRCLS_DEV_VER_T* verInfoPtr)
{
	...
	************************************************************************************/
	...
	if (status == MXL_SUCCESS)
	{
		LOG_INFO_PRINT("[HRCLS][%d] chipVersion=%d MxLWare: %d.%d.%d.%d-RC%d Firmware:%d.%d.%d.%d-RC%d Bootloader:%d.%d.%d.%d fwDownloaded=%d\n",
		HRCLS_DEVICE_ID,
	...
	************************************************************************************/
	...
	if (status != MXL_FAILURE)
	{
		...
		************************************************************************************/
		...
		if (status != MXL_SUCCESS)
		{
			LOG_INFO_PRINT("[HRCLS][%d] {%s} Fullband tuner synth lock TIMEOUT!\n", HRCLS_DEVICE_ID, __FUNCTION__);
			return MXL_FAILURE;
		}
	}
