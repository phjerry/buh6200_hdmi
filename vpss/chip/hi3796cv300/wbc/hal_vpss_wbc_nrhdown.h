...
typedef enum {
    VPSS_WBC_NRHDOWN_MODE_NORMAL = 2,  // 回写帧模式，回写队列开辟2个帧存节点
    VPSS_WBC_NRHDOWN_MODE_3FIELD = 3,  // 回写三场模式，回写队列开辟3个帧存节点
    VPSS_WBC_NRHDOWN_MODE_4FIELD = 4,  // 回写三场模式，回写队列开辟4个帧存节点
    VPSS_WBC_NRHDOWN_MODE_5FIELD = 5,  // 回写五场模式，回写队列开辟5个帧存节点
...
typedef struct {
    vpss_wbc_nrhdown_mode mode;          // 回写队列工作模式
    vpss_wbc_nrhdown_ref_mode ref_mode;  // 模式值使用模式
    hi_u32 width;                        // 帧信息宽
    hi_u32 height;                       // 帧信息高
    hi_bool interlace;
    hi_drv_pixel_format pixel_format;  // 帧信息像素格式
...
typedef struct {
    ...
    vpss_wbc_nrhdown_data data_list[VPSS_WBC_NRHDOWN_MAX_NODE];  // 回写链表结构
    vpss_wbc_nrhdown_attr wbc_attr;                              // 回写链表属性
