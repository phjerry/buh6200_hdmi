...
#define mm_ps(phase, minp, maxp, mins, maxs) do { \
        hi_u32 pluse, space, factor; \
        pluse  = (hi_u32)(phase)->pluse; \
        space  = (hi_u32)(phase)->space; \
        factor = (phase)->factor; \
...
hi_s32 ir_protocol_check(ir_protocol *ip)
{
    ...
    list_for_each(c, &ir_prot_head) {
        ci = list_entry(c, ir_protocol, node);
...
static hi_void inline init_mmps(ir_protocol *ip)
{
...
hi_s32 ir_register_protocol(ir_protocol *ip)
{
    ...
    if (ip->idx < IR_PROT_RC5) {
        if (ip->attr.second_header.pluse ||
            ...
            list_for_each(curr, &ir_prot_head) {
                p = list_entry(curr, ir_protocol, node);
...
hi_s32 ir_unregister_protocol(ir_protocol *ip)
{
    hi_ulong flag;
    spin_lock_irqsave(&irlock, flag);
    list_del(&ip->node);
...
hi_s32 ir_protocol_exit(hi_void)
{
    ...
    while (!list_empty_careful(&ir_prot_head)) {
        ip = list_first_entry(&ir_prot_head, ir_protocol, node);
        (hi_void)ir_unregister_protocol(ip);
    }
...
ir_protocol *ir_prot_next(ir_protocol *curr)
{
    ...
    c  = curr->node.next;
    ip = list_entry(c, ir_protocol, node);
    return ip;
}

hi_s32 ir_prot_valid(ir_protocol *ip)
{
    return (ip && ip->ir_code_name && ip->idx != IR_PROT_MAX);
}
