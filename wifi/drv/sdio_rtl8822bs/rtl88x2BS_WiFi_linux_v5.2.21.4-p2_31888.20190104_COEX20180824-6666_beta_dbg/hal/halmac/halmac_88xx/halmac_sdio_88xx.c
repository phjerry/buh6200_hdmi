...
)
{
	...
	status = halmac_convert_to_sdio_bus_offset_88xx(pHalmac_adapter, &reg_cfg);
	if (status != HALMAC_RET_SUCCESS)
		return status;
	status = halmac_convert_to_sdio_bus_offset_88xx(pHalmac_adapter, &reg_data);
	if (status != HALMAC_RET_SUCCESS)
		return status;

	PLATFORM_SDIO_CMD52_WRITE(pDriver_adapter, reg_cfg, (u8)halmac_offset);
	PLATFORM_SDIO_CMD52_WRITE(pDriver_adapter, reg_cfg + 1, (u8)(halmac_offset >> 8));
	PLATFORM_SDIO_CMD52_WRITE(pDriver_adapter, reg_cfg + 2, (u8)(BIT(3) | BIT(4)));

	do {
		tmp = PLATFORM_SDIO_CMD52_READ(pDriver_adapter, reg_cfg + 2);
		cnt--;
	...
	value8 = PLATFORM_SDIO_CMD52_READ(pDriver_adapter, reg_data);
...
)
{
	...
	PLATFORM_SDIO_CMD53_WRITE_32(pDriver_adapter, reg_cfg, halmac_offset | BIT(19) | BIT(20));

	do {
		PLATFORM_SDIO_CMD53_READ_N(pDriver_adapter, reg_cfg + 2, sizeof(value), value);
		cnt--;
	...
	case HALMAC_IO_WORD:
		if (HALMAC_MAC_POWER_OFF == pHalmac_adapter->halmac_state.mac_power) {
			if (0 != (halmac_offset & (2 - 1))) {
				...
				value32.byte[0] = halmac_read_indirect_cmd52_sdio_88xx(pHalmac_adapter, halmac_offset);
				value32.byte[1] = PLATFORM_SDIO_CMD52_READ(pDriver_adapter, reg_data + 1);
			}
			value32.dword = rtk_le32_to_cpu(value32.dword);
		} else {
			if (0 != (halmac_offset & (2 - 1))) {
	...
	case HALMAC_IO_DWORD:
		if (HALMAC_MAC_POWER_OFF == pHalmac_adapter->halmac_state.mac_power) {
			if (0 != (halmac_offset & (4 - 1))) {
				...
				value32.byte[0] = halmac_read_indirect_cmd52_sdio_88xx(pHalmac_adapter, halmac_offset);
				value32.byte[1] = PLATFORM_SDIO_CMD52_READ(pDriver_adapter, reg_data + 1);
				value32.byte[2] = PLATFORM_SDIO_CMD52_READ(pDriver_adapter, reg_data + 2);
				value32.byte[3] = PLATFORM_SDIO_CMD52_READ(pDriver_adapter, reg_data + 3);
			}
			value32.dword = rtk_le32_to_cpu(value32.dword);
		} else {
			if (0 != (halmac_offset & (4 - 1))) {
...
/*only for w_indir_sdio_88xx !!, Soar 20190104*/
HALMAC_RET_STATUS
halmac_write_indirect_cmd52_sdio_88xx(
	IN PHALMAC_ADAPTER pHalmac_adapter,
	IN u32 adr,
	IN u32 val,
	IN HALMAC_IO_SIZE size
)
{
	...
	PLATFORM_SDIO_CMD52_WRITE(pDriver_adapter, reg_cfg, (u8)adr);
	PLATFORM_SDIO_CMD52_WRITE(pDriver_adapter, reg_cfg + 1, (u8)(adr >> 8));
	switch (size) {
	case HALMAC_IO_BYTE:
		PLATFORM_SDIO_CMD52_WRITE(pDriver_adapter, reg_data, (u8)val);
		PLATFORM_SDIO_CMD52_WRITE(pDriver_adapter, reg_cfg + 2, (u8)(BIT(2) | BIT(4)));
		break;
	case HALMAC_IO_WORD:
		PLATFORM_SDIO_CMD52_WRITE(pDriver_adapter, reg_data, (u8)val);
		PLATFORM_SDIO_CMD52_WRITE(pDriver_adapter, reg_data + 1, (u8)(val >> 8));
		PLATFORM_SDIO_CMD52_WRITE(pDriver_adapter, reg_cfg + 2, (u8)(BIT(0) | BIT(2) | BIT(4)));
		break;
	case HALMAC_IO_DWORD:
		PLATFORM_SDIO_CMD52_WRITE(pDriver_adapter, reg_data, (u8)val);
		PLATFORM_SDIO_CMD52_WRITE(pDriver_adapter, reg_data + 1, (u8)(val >> 8));
		PLATFORM_SDIO_CMD52_WRITE(pDriver_adapter, reg_data + 2, (u8)(val >> 16));
		PLATFORM_SDIO_CMD52_WRITE(pDriver_adapter, reg_data + 3, (u8)(val >> 24));
		PLATFORM_SDIO_CMD52_WRITE(pDriver_adapter, reg_cfg + 2, (u8)(BIT(1) | BIT(2) | BIT(4)));
		break;
	default:
		break;
	}
	...
	if (((cnt & BIT(4)) == 0) && cnt == 0)
		PLATFORM_MSG_PRINT(pDriver_adapter, HALMAC_MSG_COMMON, HALMAC_DBG_ERR, "halmac_write_indirect_cmd52_sdio_88xx fail, offset = 0x%x\n", adr);
...
/*only for w_indir_sdio_88xx !!, Soar 20190104*/
HALMAC_RET_STATUS
halmac_write_indirect_cmd53_sdio_88xx(
	IN PHALMAC_ADAPTER pHalmac_adapter,
	IN u32 adr,
	IN u32 val,
	IN HALMAC_IO_SIZE size
)
{
	...
	PLATFORM_SDIO_CMD53_WRITE_32(pDriver_adapter, reg_data, val);
	PLATFORM_SDIO_CMD53_WRITE_32(pDriver_adapter, reg_cfg, value32);
	...
	if (((cnt & BIT(4)) == 0) && cnt == 0)
		PLATFORM_MSG_PRINT(pDriver_adapter, HALMAC_MSG_COMMON, HALMAC_DBG_ERR, "halmac_write_indirect_cmd53_sdio_88xx fail, offset = 0x%x\n", adr);

	return status;
}

HALMAC_RET_STATUS
halmac_write_indirect_sdio_88xx(
	IN PHALMAC_ADAPTER pHalmac_adapter,
	IN u32 adr,
	IN u32 val,
	IN HALMAC_IO_SIZE size
)
{
	...
	switch (size) {
	case HALMAC_IO_BYTE:
		if (pHalmac_adapter->pwr_off_flow_flag == 1 || pHalmac_adapter->halmac_state.mac_power == HALMAC_MAC_POWER_OFF)
			status = halmac_write_indirect_cmd52_sdio_88xx(pHalmac_adapter, adr, val, HALMAC_IO_BYTE);
		else
			status = halmac_write_indirect_cmd53_sdio_88xx(pHalmac_adapter, adr, val, HALMAC_IO_BYTE);
		break;
	case HALMAC_IO_WORD:
		if (pHalmac_adapter->halmac_state.mac_power == HALMAC_MAC_POWER_OFF) {
			if (0 != (adr & (2 - 1))) {
				status = halmac_write_indirect_cmd52_sdio_88xx(pHalmac_adapter, adr, val, HALMAC_IO_BYTE);
				status = halmac_write_indirect_cmd52_sdio_88xx(pHalmac_adapter, adr + 1, val >> 8, HALMAC_IO_BYTE);
			} else {
				status = halmac_write_indirect_cmd52_sdio_88xx(pHalmac_adapter, adr, val, HALMAC_IO_WORD);
			}
		} else {
			if (0 != (adr & (2 - 1))) {
				status = halmac_write_indirect_cmd53_sdio_88xx(pHalmac_adapter, adr, val, HALMAC_IO_BYTE);
				status = halmac_write_indirect_cmd53_sdio_88xx(pHalmac_adapter, adr + 1, val >> 8, HALMAC_IO_BYTE);
			} else {
				status = halmac_write_indirect_cmd53_sdio_88xx(pHalmac_adapter, adr, val, HALMAC_IO_WORD);
			}
		}
		break;
	case HALMAC_IO_DWORD:
		if (pHalmac_adapter->halmac_state.mac_power == HALMAC_MAC_POWER_OFF) {
			if (0 != (adr & (4 - 1))) {
				status = halmac_write_indirect_cmd52_sdio_88xx(pHalmac_adapter, adr, val, HALMAC_IO_BYTE);
				status = halmac_write_indirect_cmd52_sdio_88xx(pHalmac_adapter, adr + 1, val >> 8, HALMAC_IO_BYTE);
				status = halmac_write_indirect_cmd52_sdio_88xx(pHalmac_adapter, adr + 2, val >> 16, HALMAC_IO_BYTE);
				status = halmac_write_indirect_cmd52_sdio_88xx(pHalmac_adapter, adr + 3, val >> 24, HALMAC_IO_BYTE);
			} else {
				status = halmac_write_indirect_cmd52_sdio_88xx(pHalmac_adapter, adr, val, HALMAC_IO_DWORD);
			}
		} else {
			if (0 != (adr & (4 - 1))) {
				status = halmac_write_indirect_cmd53_sdio_88xx(pHalmac_adapter, adr, val, HALMAC_IO_BYTE);
				status = halmac_write_indirect_cmd53_sdio_88xx(pHalmac_adapter, adr + 1, val >> 8, HALMAC_IO_BYTE);
				status = halmac_write_indirect_cmd53_sdio_88xx(pHalmac_adapter, adr + 2, val >> 16, HALMAC_IO_BYTE);
				status = halmac_write_indirect_cmd53_sdio_88xx(pHalmac_adapter, adr + 3, val >> 24, HALMAC_IO_BYTE);
			} else {
				status = halmac_write_indirect_cmd53_sdio_88xx(pHalmac_adapter, adr, val, HALMAC_IO_DWORD);
			}
		}
		break;
	default:
