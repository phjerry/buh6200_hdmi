...
)
{
	...
	if ((halmac_offset & 0xFFFF0000) == 0) {
		halmac_write_indirect_sdio_88xx(pHalmac_adapter, halmac_offset, halmac_data, HALMAC_IO_BYTE);
	} else {
		status = halmac_convert_to_sdio_bus_offset_88xx(pHalmac_adapter, &halmac_offset);
		if (status != HALMAC_RET_SUCCESS) {
...
)
{
	...
	if ((halmac_offset & 0xFFFF0000) == 0) {
		status = halmac_write_indirect_sdio_88xx(pHalmac_adapter, halmac_offset, halmac_data, HALMAC_IO_WORD);
		return status;
	}
	...
	if (pHalmac_adapter->halmac_state.mac_power == HALMAC_MAC_POWER_OFF ||
	    ...
	    pHalmac_adapter->sdio_cmd53_4byte == HALMAC_SDIO_CMD53_4BYTE_MODE_W) {
...
)
{
	...
	if ((halmac_offset & 0xFFFF0000) == 0) {
		status = halmac_write_indirect_sdio_88xx(pHalmac_adapter, halmac_offset, halmac_data, HALMAC_IO_DWORD);
		return status;
	}
	...
	if (pHalmac_adapter->halmac_state.mac_power == HALMAC_MAC_POWER_OFF ||
	    (halmac_offset & (4 - 1)) !=  0) {
		PLATFORM_SDIO_CMD52_WRITE(pDriver_adapter, halmac_offset, (u8)(halmac_data & 0xFF));
		PLATFORM_SDIO_CMD52_WRITE(pDriver_adapter, halmac_offset + 1, (u8)((halmac_data >> 8) & 0xFF));
		PLATFORM_SDIO_CMD52_WRITE(pDriver_adapter, halmac_offset + 2, (u8)((halmac_data >> 16) & 0xFF));
		PLATFORM_SDIO_CMD52_WRITE(pDriver_adapter, halmac_offset + 3, (u8)((halmac_data >> 24) & 0xFF));
	} else {
