...
sint recv_decache(union recv_frame *precv_frame)
{
	...
	if (seq_ctrl == *prxseq) {
		...
		if (psta->sta_stats.duplicate_cnt % 100 == 0)
			RTW_WARN(FUNC_ADPT_FMT ": duplicate pkt, qos=%d tid=%u seq=%d frag=%d\n",
				FUNC_ADPT_ARG(adapter), pattrib->qos
				, tid, precv_frame->u.hdr.attrib.seq_num
				, precv_frame->u.hdr.attrib.frag_num);

		#ifdef DBG_RX_DROP_FRAME
		RTW_INFO("DBG_RX_DROP_FRAME "FUNC_ADPT_FMT": duplicate pkt, qos=%d tid=%d seq=%d for sta="MAC_FMT"\n"
			, FUNC_ADPT_ARG(adapter), pattrib->qos, tid
...
static int recv_process_mpdu(_adapter *padapter, union recv_frame *prframe)
{
	...
	if (pattrib->amsdu) {
		...
		if (!RTW_CANNOT_RUN(padapter)) {
			...
			if (ret != _SUCCESS) {
				#ifdef DBG_RX_DROP_FRAME
				RTW_INFO("DBG_RX_DROP_FRAME "FUNC_ADPT_FMT" indicatepkt_check fail!\n"
					, FUNC_ADPT_ARG(padapter));
				#endif
				rtw_free_recvframe(prframe, pfree_recv_queue);
				goto exit;
...
static int recv_indicatepkt_reorder(_adapter *padapter, union recv_frame *prframe)
{
	...
	if (check_indicate_seq(preorder_ctrl, pattrib->seq_num) == _FALSE) {
	...
	if (enqueue_reorder_recvframe(preorder_ctrl, prframe) == _FALSE) {
...
void rtw_reordering_ctrl_timeout_handler(void *pcontext)
{
	...
	if (preorder_ctrl->bReorderWaiting == _TRUE) {
		preorder_ctrl->bReorderWaiting = _FALSE;
		...
		RTW_WARN("%s: bReorderWaiting is _FALSE?!\n", __FUNCTION__);
	}
	_exit_critical_bh(&ppending_recvframe_queue->lock, &irql);
...
static void recv_set_iseq_before_mpdu_process(union recv_frame *rframe, u16 seq_num, const char *caller)
{
	...
	if (reorder_ctrl) {
		_enter_critical_bh(&reorder_ctrl->pending_recvframe_queue.lock, NULL);
		...
		_exit_critical_bh(&reorder_ctrl->pending_recvframe_queue.lock, NULL);
	}
#endif
}
...
static void recv_set_iseq_after_mpdu_process(union recv_frame *rframe, u16 seq_num, const char *caller)
{
	...
	if (reorder_ctrl) {
		...
		if (reorder_ctrl->indicate_seq != 0xFFFF)
