								    ...
								    u64 gpu_pfn)
{
	 ...
	 */
#ifdef CONFIG_HIG1
	if ((gpu_pfn >= KBASE_REG_ZONE_HFBC_VA_BASE) &&
		(gpu_pfn < (KBASE_REG_ZONE_HFBC_VA_BASE + KBASE_REG_ZONE_HFBC_VA_SIZE)))
		rbtree = &kctx->reg_rbtree_hfbc;
	else
#endif
...
#ifdef CONFIG_HIG1
		,
		struct kbase_va_region *hfbc_va_reg
#endif
		)
{
...
#ifdef CONFIG_HIG1
	kctx->reg_rbtree_hfbc = RB_ROOT;
	/* add hisi private region */
	if(hfbc_va_reg) {
		kbase_region_tracker_insert(hfbc_va_reg);
	}
#endif
}
...
#ifdef CONFIG_HIG1
	kbase_region_tracker_erase_rbtree(&kctx->reg_rbtree_hfbc);
#endif
}
...
int kbase_region_tracker_init(struct kbase_context *kctx)
{
...
#ifdef CONFIG_HIG1
	struct kbase_va_region *hfbc_va_reg = NULL;
#endif
	...
	if (same_va_bits == 32) {
		...
		same_va_pages = (1ULL << (same_va_bits - PAGE_SHIFT)) - 1 - KBASE_REG_ZONE_HFBC_VA_SIZE;
	}
#else
	same_va_pages = (1ULL << (same_va_bits - PAGE_SHIFT)) - 1;
#endif
...
#ifdef CONFIG_HIG1
	{
		u64 hfbc_va_size = KBASE_REG_ZONE_HFBC_VA_SIZE;
		if ((KBASE_REG_ZONE_HFBC_VA_BASE + KBASE_REG_ZONE_HFBC_VA_SIZE) >= gpu_va_limit)
			hfbc_va_size = gpu_va_limit - KBASE_REG_ZONE_HFBC_VA_BASE;

		hfbc_va_reg = kbase_alloc_free_region(&kctx->reg_rbtree_hfbc,
						KBASE_REG_ZONE_HFBC_VA_BASE,
						hfbc_va_size, KBASE_REG_ZONE_HFBC_VA);

		if (!hfbc_va_reg) {
				err = -ENOMEM;
				if(!custom_va_reg) {
						goto fail_free_custom;
				} else {
						goto fail_free_same_va;
				}
		}

		kbase_region_tracker_ds_init(kctx, same_va_reg, custom_va_reg, hfbc_va_reg);
	}
#else
...
#if defined(CONFIG_HIG1_SECURE) && !defined(CONFIG_HIG1_SECURE_SIM)
static int kbase_jd_smmu_map(struct kbase_context *kctx,
		struct kbase_va_region *reg)
{
	int i;
	struct tagged_addr *pa;
	...
	for (i = 0; i < reg->nr_pages; i++)
		*pa++ = as_tagged(alloc->imported.umm.smmu_addr +
			(i << PAGE_SHIFT));
...
static void kbase_jd_smmu_unmap(struct kbase_context *kctx,
		struct kbase_mem_phy_alloc *alloc)
{
		...
		struct kbase_va_region *reg)
{
	...
	if(reg->flags & KBASE_REG_SECURE) {
		return kbase_jd_smmu_map(kctx, reg);
	}
#endif
...
{
#if defined(CONFIG_HIG1_SECURE) && !defined(CONFIG_HIG1_SECURE_SIM)
	if(alloc->imported.umm.sgt == NULL) {
		return kbase_jd_smmu_unmap(kctx, alloc);
	}
#endif
