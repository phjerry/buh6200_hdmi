...
#ifdef CONFIG_HIG1
#define KBASE_GMMU_PAGE_SHIFT 	12
#define KBASE_GMMU_VALID_MASK	0x0FFFFFFFFF

#if defined(CONFIG_HIG1_TILE_64X4_V0)      /* 64x4 v1r0 */
#define KBASE_GMMU_SET_ATE(addr, vpfn, s, c, as_nr)	\
	do {\
		addr = (((vpfn) << KBASE_GMMU_PAGE_SHIFT) & KBASE_GMMU_VALID_MASK) | ((u64)(((s) << 4) | ((c) << 3) | (as_nr)) << 35);\
	}while(0)
#elif defined(CONFIG_HIG1_TILE_64X4_V1) /* 64x4 v1r2 */
#define KBASE_GMMU_SET_ATE(addr, vpfn, s, c, as_nr)	\
	do {\
		addr = (((vpfn) << KBASE_GMMU_PAGE_SHIFT) & KBASE_GMMU_VALID_MASK) | ((u64)(((c) << 4) | ((as_nr) << 1) | (s)) << 35);\
	}while(0)
#else                      /* 64x2 */
#define KBASE_GMMU_SET_ATE(addr, vpfn, s, c, as_nr)	\
	do {\
		addr = (((vpfn) << KBASE_GMMU_PAGE_SHIFT) & KBASE_GMMU_VALID_MASK) | ((u64)(((c) << 3) | (as_nr)) << 36);\
	}while(0)
#endif
#endif
/**
...
#ifdef CONFIG_HIG1
static int hmmu_get_bottom_pgd(struct kbase_device *kbdev,
		struct kbase_mmu_table *mmut,
		u64 vpfn,
		phys_addr_t *out_pgd)
{
	...
	*out_pgd = mmut->hmmu.pgd_hmmu;

	err = mmu_get_next_pgd(kbdev, mmut, out_pgd, vpfn, 2);
	/* Handle failure condition */
	if (err) {
		dev_dbg(kbdev->dev, "hmmu_get_bottom_pgd: mmu_get_next_pgd failure\n");
		return err;
	}
...
#ifdef CONFIG_HIG1
int kbase_mmu_insert_single_secure_page(struct kbase_context *kctx, u64 vpfn,
					struct tagged_addr phys, size_t nr,
					unsigned long flags)
{
	phys_addr_t pgd;
	...
	while (remain) {
		...
		for (i = 0; i < count; i++) {
            ...
            KBASE_GMMU_SET_ATE(phys_gmmu.tagged_addr, vpfn - KBASE_REG_ZONE_HFBC_VA_BASE, 1, flags & KBASE_REG_HFBC ? 1 : 0, kctx->as_nr & 0x7);
            mmu_mode->entry_set_ate(&pgd_page[ofs],
                        phys_gmmu, flags,
                        MIDGARD_MMU_BOTTOMLEVEL);
            kctx->mmu.hmmu.pgd_smmu_kvirt[vpfn - KBASE_REG_ZONE_HFBC_VA_BASE] = (u32)phys.tagged_addr;
#else
			KBASE_GMMU_SET_ATE(phys_gmmu.tagged_addr, phys.tagged_addr >> KBASE_GMMU_PAGE_SHIFT, 1, flags & KBASE_REG_HFBC ? 1 : 0, kctx->as_nr & 0x7);
			mmu_mode->entry_set_ate(&pgd_page[ofs],
						phys_gmmu, flags,
						MIDGARD_MMU_BOTTOMLEVEL);
#endif
        ...
        kbase_mmu_sync_pgd(kctx->kbdev,
            kctx->mmu.hmmu.pgd_smmu + (vpfn - KBASE_REG_ZONE_HFBC_VA_BASE) * 4,
            sizeof(u32));
#endif
		kbase_mmu_sync_pgd(kctx->kbdev,
...
int kbase_mmu_insert_single_hig1_page(struct kbase_context *kctx, u64 vpfn,
					struct tagged_addr phys, size_t nr,
					unsigned long flags)
{
	phys_addr_t pgd;
	...
	printk("HIG1_DBG:mmu=%p, as_nr=%d, %s\n", &kctx->mmu, kctx->as_nr, __func__);
#endif
	...
	while (remain) {
		...
		do {
			err = mmu_get_bottom_pgd(kctx->kbdev, &kctx->mmu,
					vpfn, &pgd);
			err = hmmu_get_bottom_pgd(kctx->kbdev, &kctx->mmu,
				vpfn, &pgd_hmmu);
			if (err != -ENOMEM)
				break;
			/* Fill the memory pool with enough pages for
			...
			mutex_unlock(&kctx->mmu.mmu_lock);
			err = kbase_mem_pool_grow(&kctx->kbdev->mem_pool,
					MIDGARD_MMU_BOTTOMLEVEL + MIDGARD_HMMU_BOTTOMLEVEL);
			mutex_lock(&kctx->mmu.mmu_lock);
		} while (!err);
		if (err) {
			dev_warn(kctx->kbdev->dev, "kbase_mmu_insert_pages: mmu_get_bottom_pgd failure\n");
		...
		p_hmmu = pfn_to_page(PFN_DOWN(pgd_hmmu));
		pgd_page_hmmu = kmap(p_hmmu);
		if (!pgd_page_hmmu) {
			dev_warn(kctx->kbdev->dev, "kbase_mmu_insert_pages: kmap failure\n");
			if (recover_required) {
				/* Invalidate the pages we have partially
		...
		for (i = 0; i < count; i++) {
			...
			KBASE_GMMU_SET_ATE(phys_gmmu.tagged_addr, (vpfn + i), 0, 1, kctx->as_nr & 0x7);
			mmu_mode->entry_set_ate(&pgd_page[ofs],
				phys_gmmu, flags, MIDGARD_MMU_BOTTOMLEVEL);
			mmu_mode->entry_set_ate(&pgd_page_hmmu[ofs],
						phys, flags,
						MIDGARD_HMMU_BOTTOMLEVEL);
		}
		...
		kbase_mmu_sync_pgd(kctx->kbdev,
				...
				count * sizeof(u64));
		kbase_mmu_sync_pgd(kctx->kbdev,
				kbase_dma_addr(p_hmmu) + (index * sizeof(u64)),
				count * sizeof(u64));
...
#ifdef CONFIG_HIG1
	if(flags & KBASE_REG_SECURE) {
		return kbase_mmu_insert_single_secure_page(kctx, vpfn,
					phys, nr, flags);
	} else if(flags & KBASE_REG_HFBC) {
		return kbase_mmu_insert_single_hig1_page(kctx, vpfn,
					phys, nr, flags);
	}
#endif
...
#ifdef CONFIG_HIG1
int kbase_mmu_insert_secure_pages_no_flush(struct kbase_device *kbdev,
				    struct kbase_mmu_table *mmut,
	...
	while (remain) {
		...
		if (cur_level == MIDGARD_MMU_LEVEL(2)) {
			...
			if (mmu_mode->pte_is_valid(*target, cur_level))
				...
				KBASE_GMMU_SET_ATE(phys_gmmu.tagged_addr, phys->tagged_addr >> KBASE_GMMU_PAGE_SHIFT, 1, flags & KBASE_REG_HFBC ? 1 : 0, mmut->kctx->as_nr & 0x7);
			mmu_mode->entry_set_ate(target, phys_gmmu, flags,
						cur_level);
		} else {
			for (i = 0; i < count; i++) {
...
#ifdef CONFIG_HIG1_SECURE_SIM
				KBASE_GMMU_SET_ATE(phys_gmmu.tagged_addr, start_vpfn - KBASE_REG_ZONE_HFBC_VA_BASE + i, 1, flags & KBASE_REG_HFBC ? 1 : 0, mmut->kctx->as_nr & 0x7);
				kbdev->mmu_mode->entry_set_ate(target,
						phys_gmmu, flags, cur_level);
                mmut->hmmu.pgd_smmu_kvirt[start_vpfn - KBASE_REG_ZONE_HFBC_VA_BASE + i] = (u32)phys[i].tagged_addr;
#else
				KBASE_GMMU_SET_ATE(phys_gmmu.tagged_addr, phys[i].tagged_addr >> KBASE_GMMU_PAGE_SHIFT, 1, flags & KBASE_REG_HFBC ? 1 : 0, mmut->kctx->as_nr & 0x7);
				kbdev->mmu_mode->entry_set_ate(target,
						phys_gmmu, flags, cur_level);
#endif
			}
...
#ifdef CONFIG_HIG1_SECURE_SIM
        kbase_mmu_sync_pgd(kbdev,
            mmut->hmmu.pgd_smmu + (start_vpfn - KBASE_REG_ZONE_HFBC_VA_BASE) * 4,
            count * sizeof(u32));
#endif
		kbase_mmu_sync_pgd(kbdev,
...
int kbase_mmu_insert_hig1_pages_no_flush(struct kbase_device *kbdev,
				    struct kbase_mmu_table *mmut,
				    const u64 start_vpfn,
				    struct tagged_addr *phys, size_t nr,
				    unsigned long flags)
{
...
#ifdef CONFIG_HIG1_DEBUG
	printk("HIG1_DBG:mmu=%p, as_nr=%d, vpfn=%llx %s\n", mmut, mmut->kctx->as_nr, start_vpfn, __func__);
#endif
	...
	while (remain) {
		...
		do {
			err = mmu_get_pgd_at_level(kbdev, mmut, insert_vpfn,
						   cur_level, &pgd);
			err |= hmmu_get_bottom_pgd(kbdev, mmut, insert_vpfn, &pgd_hmmu);
			...
			mutex_unlock(&mmut->mmu_lock);
			err = kbase_mem_pool_grow(&kbdev->mem_pool,
					cur_level + MIDGARD_HMMU_BOTTOMLEVEL);
			mutex_lock(&mmut->mmu_lock);
		} while (!err);
		...
		if (cur_level == MIDGARD_MMU_LEVEL(2)) {
			...
			for (i = 0; i < count; i++) {
				...
				KBASE_GMMU_SET_ATE(phys_gmmu.tagged_addr, insert_vpfn + i, 0, 1, mmut->kctx->as_nr & 0x7);
				mmu_mode->entry_set_ate(&pgd_page[ofs],
					phys_gmmu, flags, cur_level);
				mmu_mode->entry_set_ate(&pgd_page_hmmu[ofs],
		...
		kbase_mmu_sync_pgd(kbdev,
				...
				count * sizeof(u64));
		kbase_mmu_sync_pgd(kbdev,
				kbase_dma_addr(p_hmmu) + (vindex * sizeof(u64)),
				count * sizeof(u64));
...
#ifdef CONFIG_HIG1
	if(flags & KBASE_REG_SECURE) {
		return kbase_mmu_insert_secure_pages_no_flush(kbdev,
				    mmut, start_vpfn, phys, nr, flags);
	} else if(flags & KBASE_REG_HFBC) {
		return kbase_mmu_insert_hig1_pages_no_flush(kbdev,
				    mmut, start_vpfn, phys, nr, flags);
	}
#endif
...
#ifdef CONFIG_HIG1
void kbase_mmu_update_asid(struct kbase_device *kbdev,
		struct kbase_mmu_table *mmut,
		int as_nr)
{
	lockdep_assert_held(&kbdev->hwaccess_lock);
    ...
    kbase_mmu_mode_update_asid(kbdev, mmut, as_nr);
}
#endif
...
#ifdef CONFIG_HIG1
static int kbase_mmu_update_secure_pages_no_flush(struct kbase_context *kctx, u64 vpfn,
					struct tagged_addr *phys, size_t nr,
					unsigned long flags)
{
	struct tagged_addr phys_smmu;
	phys_addr_t pgd;
	u64 *pgd_page;
	struct kbase_mmu_mode const *mmu_mode;
	int err;
	...
	while (nr) {
		...
		for (i = 0; i < count; i++) {
#ifdef CONFIG_HIG1_SECURE_SIM
			KBASE_GMMU_SET_ATE(phys_gmmu.tagged_addr, vpfn - KBASE_REG_ZONE_HFBC_VA_BASE + i, 1, flags & KBASE_REG_HFBC ? 1 : 0, kctx->as_nr & 0x7);
			mmu_mode->entry_set_ate(&pgd_page[index + i], phys_gmmu,
						flags, MIDGARD_MMU_BOTTOMLEVEL);

            kctx->mmu.hmmu.pgd_smmu_kvirt[vpfn - KBASE_REG_ZONE_HFBC_VA_BASE + i] = (u32)phys[i].tagged_addr;
#else
			KBASE_GMMU_SET_ATE(phys_smmu.tagged_addr, phys[i].tagged_addr >> KBASE_GMMU_PAGE_SHIFT, 1, flags & KBASE_REG_HFBC ? 1 : 0, kctx->as_nr & 0x7);
			mmu_mode->entry_set_ate(&pgd_page[index + i], phys_smmu,
						flags, MIDGARD_MMU_BOTTOMLEVEL);
#endif
		}
...
static int kbase_mmu_update_hig1_pages_no_flush(struct kbase_context *kctx, u64 vpfn,
					struct tagged_addr *phys, size_t nr,
					unsigned long flags)
{
	phys_addr_t pgd;
	...
	while (nr) {
		...
		for (i = 0; i < count; i++) {
			KBASE_GMMU_SET_ATE(phys_gmmu.tagged_addr, vpfn + i, 0, 1, kctx->as_nr & 0x7);
			mmu_mode->entry_set_ate(&pgd_page[index + i],
				phys_gmmu, flags, MIDGARD_HMMU_BOTTOMLEVEL);
			mmu_mode->entry_set_ate(&pgd_page_hmmu[index + i],
...
#ifdef CONFIG_HIG1
	if(flags & KBASE_REG_SECURE) {
		return kbase_mmu_update_secure_pages_no_flush(kctx, vpfn,
					phys, nr, flags);
	} else if(flags & KBASE_REG_HFBC) {
		return kbase_mmu_update_hig1_pages_no_flush(kctx, vpfn,
					phys, nr, flags);
	}
#endif
...
#ifdef CONFIG_HIG1
int kbase_hmmu_init(struct kbase_device *kbdev, struct kbase_mmu_table *mmut,
		struct kbase_context *kctx)
{
	mmut->hmmu.pgd_hmmu= 0;
	mmut->hmmu.ice_desc_mask = 0;
	mmut->hmmu.ice_desc_hw_mask = 0;
	mmut->hmmu.ice_desc_changed = false;
	mmut->hmmu.page_ice_desc = NULL;
	mmut->hmmu.virt_ice_desc = NULL;
	mmut->hmmu.asid_changed = false;

	/* We allocate pages into the kbdev memory pool, then
	 * kbase_mmu_alloc_pgd will allocate out of that pool. This is done to
	 * avoid allocations from the kernel happening with the lock held.
	 */
	while (!mmut->hmmu.pgd_hmmu) {
		int err;

		err = kbase_mem_pool_grow(&kbdev->mem_pool,
				MIDGARD_HMMU_BOTTOMLEVEL + 1);
		if (err) {
			kbase_hmmu_term(kbdev, mmut);
			return -ENOMEM;
		}

		mutex_lock(&mmut->mmu_lock);
		mmut->hmmu.pgd_hmmu = kbase_mmu_alloc_pgd(kbdev, mmut);
		if(0 == mmut->hmmu.pgd_hmmu) {
			dev_err(kbdev->dev, "HIG1:pgd_hmmu=0x%x, kbase_mmu_alloc_pgd failed\n", (u32)mmut->hmmu.pgd_hmmu);
			mutex_unlock(&mmut->mmu_lock);
			kbase_hmmu_term(kbdev, mmut);
			return -ENOMEM;
		}
		
		mmut->hmmu.phy_ice_desc = kbase_mmu_alloc_pgd(kbdev, mmut);
		if(0 == mmut->hmmu.phy_ice_desc) {
			dev_err(kbdev->dev, "HIG1:pgd_hmmu=0x%x, kbase_mmu_alloc_pgd failed\n", (u32)mmut->hmmu.pgd_hmmu);
			mutex_unlock(&mmut->mmu_lock);
			kbase_hmmu_term(kbdev, mmut);
			return -ENOMEM;
		}
		mmut->hmmu.page_ice_desc = pfn_to_page(PFN_DOWN(mmut->hmmu.phy_ice_desc));
		mmut->hmmu.virt_ice_desc = (u8 *)kmap(mmut->hmmu.page_ice_desc);

#ifdef HIG1_DEBUG
		dev_warn(kbdev->dev, "HIG1_DBG:pgd_hmmu = 0x%x, phy_ice_desc=0x%x\n", (u32)mmut->hmmu.pgd_hmmu, (u32)mmut->hmmu.phy_ice_desc);
		printk("HIG1_DBG:mmut = %p, pgd=0x%x, pgd_hmmu = 0x%x, phy_ice_desc=0x%x\n", mmut, (u32)mmut->pgd, (u32)mmut->hmmu.pgd_hmmu, (u32)mmut->hmmu.phy_ice_desc);
#endif
...
void kbase_hmmu_term(struct kbase_device *kbdev, struct kbase_mmu_table *mmut)
{
#ifdef HIG1_DEBUG
	printk("HIG1_DBG:%s, mmut=%p\n", __func__, mmut);
#endif
	if (mmut->hmmu.pgd_hmmu) {
		mutex_lock(&mmut->mmu_lock);
		mmu_teardown_level(kbdev, mmut, mmut->hmmu.pgd_hmmu, MIDGARD_MMU_TOPLEVEL,
				mmut->mmu_teardown_pages);

		/* release phy_ice_desc */
		kunmap(mmut->hmmu.page_ice_desc);
		mmut->hmmu.virt_ice_desc = NULL;
		kbase_mem_pool_free(&kbdev->mem_pool, mmut->hmmu.page_ice_desc, true);
		mmut->hmmu.page_ice_desc = NULL;
...
#ifdef CONFIG_HIG1_SECURE_SIM
//allocate 1MB memory as the smmu table
int kbase_smmu_init(struct kbase_device *kbdev, struct kbase_mmu_table *mmut,
		struct kbase_context *kctx)
{
    struct page *p;
    gfp_t gfp;
    ...
    dma_addr = dma_map_page(dev, p, 0, (PAGE_SIZE << 8),
                DMA_BIDIRECTIONAL);
    ...
    mmut->hmmu.pgd_smmu = dma_addr;
    mmut->hmmu.pgd_smmu_page = p;
    mmut->hmmu.pgd_smmu_kvirt = kmap(p);

    return 0;
}

void kbase_smmu_term(struct kbase_device *kbdev, struct kbase_mmu_table *mmut)
{
    int i;

    kunmap(mmut->hmmu.pgd_smmu_page);
    
	dma_unmap_page(kbdev->dev, mmut->hmmu.pgd_smmu, (PAGE_SIZE << 8),
		       DMA_BIDIRECTIONAL);
	for (i = 0; i < (1u << 8); i++)
		kbase_clear_dma_addr(mmut->hmmu.pgd_smmu_page+i);

	__free_pages(mmut->hmmu.pgd_smmu_page, 8);
}
#endif
#endif
