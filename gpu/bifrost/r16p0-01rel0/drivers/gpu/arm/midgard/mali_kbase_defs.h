...
#ifdef CONFIG_HIG1
#define MIDGARD_HMMU_BOTTOMLEVEL MIDGARD_MMU_LEVEL(1)
#endif
...
#ifdef CONFIG_HIG1
struct kbase_icedesc_slot {
	u8 idx;
	u64 jc;
};

/* support max 8 HiG1 textures per asid on hardware */
#define HIG1_MAX_TEXTURE_PER_HW_ASID 8

struct kbase_hmmu_table {
	phys_addr_t pgd_hmmu;       /* pgd for hig1 */
	phys_addr_t phy_ice_desc;   /* physical address for ice descriptor */
	struct page *page_ice_desc; /* ice_desc page pointer */
	u8 *virt_ice_desc;          /* ice_desc virt pointer */
	u32 asid_changed;           /* whether asid had changed before mmu_update */
	u32 ice_desc_mask;          /* indicate how many ice textures created by this app */
	u32 ice_desc_hw_mask;       /* indicate how many hardware textures are using by all jobs */
	struct kbase_icedesc_slot ice_desc_slot[HIG1_MAX_TEXTURE_PER_HW_ASID];
	u32 ice_desc_changed;       /* 1:changed; otherwise stay unchandged */
#ifdef CONFIG_HIG1_SECURE_SIM
	phys_addr_t pgd_smmu;       /* pgd for smmu */
	struct page *pgd_smmu_page;
	u32 *pgd_smmu_kvirt;        /* virt addr for smmu table */
#endif
};
#endif
...
void kbase_mmu_mode_update_asid(struct kbase_device *kbdev,
			struct kbase_mmu_table *mmut,
			int as_nr);

int gmmu_update_asid(struct kbase_device *kbdev,
		     struct kbase_mmu_table *mmut, int as_nr);
...
#ifdef CONFIG_HIG1
	struct rb_root reg_rbtree_hfbc;
#endif
