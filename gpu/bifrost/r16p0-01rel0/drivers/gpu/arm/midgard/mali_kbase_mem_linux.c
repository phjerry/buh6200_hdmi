		...
		int fd, u64 *va_pages, u64 *flags, u32 padding)
{
	...
	sgt = dma_buf_map_attachment(dma_attachment, DMA_BIDIRECTIONAL);
	if (!sgt)
		goto no_sgt;
...
#ifdef CONFIG_HIG1
	if (*flags & (BASE_MEM_HFBC | BASE_MEM_SECURE)) {
		reg = kbase_alloc_free_region(&kctx->reg_rbtree_hfbc,
				0, *va_pages, KBASE_REG_ZONE_HFBC_VA);
		/* Set compress flag */
		if (*flags & BASE_MEM_HFBC)
			reg->flags |= KBASE_REG_HFBC;
#ifdef CONFIG_HIG1_DEBUG
		printk("HIG1: alloc from HFBC VA in %s\n", __func__);
#endif
	} else
#endif
	if (shared_zone) {
...
#ifdef CONFIG_HIG1_SECURE_FPGA
    reg->gpu_alloc->imported.umm.smmu_addr = smmu_addr;
#endif
	reg->gpu_alloc->type = KBASE_MEM_TYPE_IMPORTED_UMM;
	reg->gpu_alloc->imported.umm.sgt = sgt;
...
no_region:
bad_size:
	dma_buf_unmap_attachment(dma_attachment, sgt, DMA_BIDIRECTIONAL);
no_sgt:
...
#ifdef CONFIG_HIG1_DEBUG
	printk("HIG1: alloc from %s\n", shared_zone ? "same va" : "custom va");
#endif
		...
		u64 *flags)
{
	...
	if(*flags & (BASE_MEM_HFBC| BASE_MEM_SECURE)) {
		...
		*gpu_va = reg->start_pfn << PAGE_SHIFT;
#ifdef CONFIG_HIG1_DEBUG
		printk("HIG1:%s, alloc from hfbc_VA, gpu_va=%llx\n", __func__, *gpu_va);
#endif
	} else
#endif
	/* mmap needed to setup VA? */
