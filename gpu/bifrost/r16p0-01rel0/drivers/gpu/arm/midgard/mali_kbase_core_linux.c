...
#ifdef CONFIG_HIG1
/* Module parameter to control mmu dump */
int mali_mmu_dump = 0;
module_param(mali_mmu_dump, int, S_IRUSR | S_IWUSR | S_IWGRP | S_IRGRP | S_IROTH); /* rw-rw-r-- */
MODULE_PARM_DESC(mali_mmu_dump, "whether enable mmu dump");
#endif
...
#ifdef CONFIG_HIG1
static int kbase_api_ice_descriptor_update(struct kbase_context *kctx,
		struct kbase_ioctl_ice_descriptor_update *update)
{
	extern int kbase_ice_desc_update(struct kbase_context *kctx, u32 valid, u32 secure, struct eglp_hiice_desc *descriptor);
	int err = 0;

	mutex_lock(&kctx->jctx.lock);

	if (kbase_ice_desc_update(kctx, update->valid, update->secure, &update->desc))
		err = -EINVAL;
...
#ifdef CONFIG_HIG1
	case KBASE_IOCTL_ICE_DESCRIPTOR_UPDATE:
		KBASE_HANDLE_IOCTL_IN(KBASE_IOCTL_ICE_DESCRIPTOR_UPDATE,
				kbase_api_ice_descriptor_update,
				struct kbase_ioctl_ice_descriptor_update);
		break;
#endif
...
#ifdef CONFIG_GPU_HISILICON
#define UTIL_MAX 100
#define UTIL_DIVISOR 10
/**
 * kbase_show_gpu_work_state - Show callback for the work_state sysfs entry.
 * @dev: The device this sysfs file is for.
 * @attr: The attributes of the sysfs file.
 * @buf: The output buffer to receive the GPU information.
 *
 * This function is called to print current GPU working status via the work_state
 * sysfs entry.  This includes the number of cores, the current frequency and
 * voltage, the current workload(in utilization) and the current power and DVFS
 * status
 *
 * Return: The number of bytes output to @buf.
 */
static ssize_t kbase_show_gpu_work_state(struct device *dev,
				  struct device_attribute *attr, char *buf)
{
	struct kbase_device *kbdev;
	...
	ret += scnprintf(buf + ret, PAGE_SIZE - ret,
			"Core Num                 :%d\n",
			kbase_get_core_num(kbdev->current_core_mask));
	ret += scnprintf(buf + ret, PAGE_SIZE - ret,
			"Frequency                :%ld(kHz)\n",
			kbdev->current_freq);
	ret += scnprintf(buf + ret, PAGE_SIZE - ret,
	...
	ret += scnprintf(buf + ret, PAGE_SIZE - ret,
			"Utilization              :%lu(percent)\n",
			UTIL_MAX * (kbdev->devfreq->last_status.busy_time >> UTIL_DIVISOR) /
				(kbdev->devfreq->last_status.total_time >> UTIL_DIVISOR));
#endif
	ret += scnprintf(buf + ret, PAGE_SIZE - ret,
			"Power Status             :%s\n",
			kbase_get_power_status() ? "power up" : "power down");
	ret += scnprintf(buf + ret, PAGE_SIZE - ret,
			"DVFS Status              :%s\n",
			config->dvfs_enable ? "enable" : "disable");

	return ret;
}
static DEVICE_ATTR(work_state, S_IRUGO, kbase_show_gpu_work_state, NULL);
#endif
...
/* because gpu depend on common driver, should init gpu driver later than common driver */
