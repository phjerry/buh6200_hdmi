					...
					enum kbasep_pm_action action)
{
	...
	/* must config power ACK mode after gpu reset, when GPU disable protected mode */
	if (action == ACTION_PWRON) {
		kbase_reg_write(kbdev, GPU_CONTROL_REG(POWER_CONTROL_MODE), cores);
	}
#endif
