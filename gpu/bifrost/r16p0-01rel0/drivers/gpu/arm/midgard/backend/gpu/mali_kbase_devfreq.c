...
kbase_devfreq_target(struct device *dev, unsigned long *target_freq, u32 flags)
{
	...
	kbase_voltage_update(freq, &voltage);
#endif
...
#ifdef CONFIG_GPU_HISILICON
	err = kbase_init_mask_freq_volt(kbdev);
	if (err) {
		dev_err(kbdev->dev,"kbase_init_mask_freq_volt failed\n");
		return err;
	}

	kbdev->devfreq = devfreq_add_device(kbdev->dev, dp,
				"bifrost_ondemand", NULL);
	if (IS_ERR(kbdev->devfreq)) {
		kfree(dp->freq_table);
		return PTR_ERR(kbdev->devfreq);
	}

	kbase_init_min_max_freq(kbdev);
#else
	kbdev->devfreq = devfreq_add_device(kbdev->dev, dp,
