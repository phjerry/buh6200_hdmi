				...
				int js)
{
...
#ifdef CONFIG_HIG1
	if(js == 0) {
		if(kctx->mmu.hmmu.ice_desc_mask && kctx->mmu.hmmu.asid_changed) {
			/* update the asid in GMMU if the asid changed and there is HFBC texture */
			gmmu_update_asid(kbdev, &kctx->mmu, kctx->as_nr);
			kctx->mmu.hmmu.asid_changed = false;
		}

		kbase_ice_desc_update_by_job(&kctx->mmu.hmmu, katom->nr_extres, katom->extres, jc_head);

		kbase_hmmu_setup(kbdev, &kctx->mmu, kctx->as_nr);
	}
#endif
	...
	printk("HIG1_DBG:%s, as_nr=%d, js%d, %s,  mmut=%p\n", __func__, katom->kctx->as_nr, js, kctx->mmu.hmmu.ice_desc_mask ? "hig1" : "non-hig1", &kctx->mmu);
#endif
}
