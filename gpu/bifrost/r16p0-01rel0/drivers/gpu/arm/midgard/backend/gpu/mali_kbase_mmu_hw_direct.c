		...
		unsigned int as_nr)
{
	...
	if (max_loops == 0) {
		dev_err(kbdev->dev, "AS_ACTIVE bit stuck, as_nr =%d\n", as_nr);
		return -1;
	}
...
static int write_cmd(struct kbase_device *kbdev, int as_nr, u32 cmd)
{
	...
	if (status == 0)
		...
		printk("HIG1_DBG:AS_ACTIVE bit stuck, as_nr =%d, cmd = %d\n", as_nr, cmd);
#endif
...
#ifdef CONFIG_HIG1
static int mmu_is_hfbc_addr(u64 vpfn)
{
	return (vpfn >= KBASE_REG_ZONE_HFBC_VA_BASE) && (vpfn < (KBASE_REG_ZONE_HFBC_VA_BASE + KBASE_REG_ZONE_HFBC_VA_SIZE)) ? 1 : 0;
}

static u32 kbase_reg_read_all(struct kbase_device *kbdev)
{
	u32 i;
	u32 val;
	...
	for(i = 0; i < 0x8000; i += 4) {
		val = readl(kbdev->reg + i);
		if(val != 0)
			printk("r: reg %08x val %08x\n", i, val);
	}
...
#ifdef CONFIG_HIG1
		if(ret) {
			struct kbase_context *kctx = kbdev->as_to_kctx[as->number];
			struct kbase_mmu_table *mmut = &kctx->mmu;
			printk("HIG1_DBG:AS_ACTIVE bit stuck, as_nr =%d, op=%d, %s, vpfn=%llx, nr = %d, lock_addr=%llx\n", as->number, op, mmu_is_hfbc_addr(vpfn) ? "HFBC" : "NON-HFBC", vpfn, nr, lock_addr);
			printk("HIG1_DBG:ice_desc_mask=%x\n", mmut->hmmu.ice_desc_mask);
			dump_stack();
			mmu_dump_gmmu_hmmu(mmut);
			kbase_reg_read_all(kbdev);
		}
#endif
